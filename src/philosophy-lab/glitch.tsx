...
/**
 * Hover-glitch manifesto: the text is drawn into a texture and rendered by a
 * shader that bends it under the cursor — the same push / wake / spring model
 * as the hero's fluid backdrop (src/hero-animated/AnimatedAtmosphere.tsx) —
 * and splits its colour channels in proportion to how far each pixel is
...
const PAD = 70 // canvas overhang around the text, so bends aren't clipped
const MAX_WAKE = 16
...
function GlitchText({ config }: { config: Partial<Config> }) {
  ...
  useEffect(() => {
    ...
    const rebuild = () => {
      ...
      W = text.offsetWidth + PAD * 2
      H = text.offsetHeight + PAD * 2
      ...
      placed = [...text.querySelectorAll<HTMLElement>('[data-w]')].map((s) => ({
        ...
        baseline: s.offsetTop + (lineH - (asc + desc)) / 2 + asc + PAD,
      }))
      paintText(revealP < 0 ? readReveal() : revealP)
    }

    /* Reveal progress of the text container, 0 (entering) to 1 (centred). */
    const container = text.closest('.plab-inner') as HTMLElement
    const readReveal = () => {
      if (!cfg.reveal) return 1
    ...
    // Config is fixed per variant; the variant remounts on switch.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  return (
    <section ref={sectionRef} className="content-section philosophy-section plab-section glitch-section">
      <div className="section-container philosophy-manifesto-container plab-inner">
