...
const getFontValue = (value?: string | number) =>
  typeof value === 'number' ? `${value}px` : value || 'inherit'
...
const buildTextCanvas = ({ container, width, height, dpr, props }: BuildCanvasOptions) => {
  ...
  if (!Number.isFinite(lineHeight)) {
    lineHeight = fontSizePx * (typeof props.lineHeight === 'number' ? props.lineHeight : 1.15)
  }
  probe.remove()
  ...
  ctx.fillStyle = props.color || '#0d111c'
  ...
  // Handle multi-line wrap gracefully for paragraphs
  const rawParagraphs = String(props.text || '').split('\n')
  const lines: string[] = []
...
export default function WarpText({
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const pVal = program.uniforms.uPointer.value as Float32Array
      pVal[0] = pointer.x
      pVal[1] = pointer.y
